...
/* FUJITSU:2011-04-21 Modified for GB start */
/*===========================================================================

FUNCTION smem_alloc_vendor1
...
===========================================================================*/
void *smem_alloc_vendor1(smem_mem_vendor1_type smem_v1_type)
{
	void 						*ret_ptr = NULL;
	smem_vendor1_data_type		*smem_ptr = NULL;

	if (( smem_v1_type <= SMEM_TSB_START_ID_V1 ) ||
		( smem_v1_type >= SMEM_TSB_LAST_ID_V1  )) {
        pr_err("error: Bad number.\n");
		return NULL;
	}

	smem_ptr = (smem_vendor1_data_type *)smem_alloc( SMEM_ID_VENDOR1,sizeof(smem_vendor1_data_type) );
	if( smem_ptr == NULL ) {
	...
	switch (smem_v1_type) {
	case SMEM_OEM_001:
		ret_ptr = &(smem_ptr->smem_v1_oem_001[0]);
	    break;
	case SMEM_OEM_002:
		ret_ptr = &(smem_ptr->smem_v1_oem_002);
	    break;
	case SMEM_OEM_003:
		ret_ptr = &(smem_ptr->smem_v1_oem_003);
	    break;
	case SMEM_OEM_004:
		ret_ptr = &(smem_ptr->smem_v1_oem_004);
	    break;
	case SMEM_OEM_005:
		ret_ptr = &(smem_ptr->smem_v1_oem_005);
	    break;
	case SMEM_OEM_006:
		ret_ptr = &(smem_ptr->smem_v1_oem_006);
	    break;
	case SMEM_OEM_007:
		ret_ptr = &(smem_ptr->smem_v1_oem_007[0]);
	    break;
	case SMEM_OEM_008:
		ret_ptr = &(smem_ptr->smem_v1_oem_008[0]);
	    break;
	case SMEM_OEM_009:
		ret_ptr = &(smem_ptr->smem_v1_oem_009[0]);
	    break;
	case SMEM_OEM_010:
		ret_ptr = &(smem_ptr->smem_v1_oem_010);
	    break;
	case SMEM_OEM_011:
		ret_ptr = &(smem_ptr->smem_v1_oem_011);
	    break;
/* FUJITSU:2011-05-24 add start */
	case SMEM_OEM_012:
		ret_ptr = &(smem_ptr->smem_v1_oem_012);
	    break;
/* FUJITSU:2011-05-24 add end */
	default:
	    /* DO NOTHING */
	    break;
	}
...
/* FUJITSU:2011-04-21 Modified for GB end */
