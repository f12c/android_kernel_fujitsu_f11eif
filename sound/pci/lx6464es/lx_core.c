...
int lx_dsp_es_check_pipeline(struct lx6464es *chip)
{
	...
	for (i = 0; i != CSES_TIMEOUT; ++i) {
		/*
		 * le bit CSES_UPDATE_LDSV est  1 ds que le macprog
		 * est pret. il re-passe  0 lorsque le premier read a
		 * t fait. pour l'instant on retire le test car ce bit
		 * passe a 1 environ 200  400 ms aprs que le registre
		 * confES  t crit (kick du xilinx ES).
		 *
...
#define IRQCS_ACTIVE_PCIDB  0x00002000L         /* Bit nÅC 13 */
#define IRQCS_ENABLE_PCIIRQ 0x00000100L         /* Bit nÅC 08 */
#define IRQCS_ENABLE_PCIDB  0x00000200L         /* Bit nÅC 09 */
