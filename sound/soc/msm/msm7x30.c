				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	MM_DBG("[ALSA] msm_dual_mic_get: session id = %d, config = %ld\n", enc_session_id,
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	MM_DBG("[ALSA] msm_dual_mic_put: session id = %d, config = %d\n", enc_session_id,
					dual_mic_config);
	return  0;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	MM_DBG("[ALSA] msm_device_mute_put: Muting device id %d(%s)\n", dev_id, dev_info->name);
