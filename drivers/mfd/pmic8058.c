...
extern int gpio_read_from_keypad;
#endif
...
#ifdef CONFIG_MACH_F11EIF
    if(system_rev >= 0x0b) {
        if(gpio_read_from_keypad != true) {
            mutex_lock(&chip->pm_lock);
        }
        else {
            if(mutex_trylock(&chip->pm_lock) != 1) {
                return 0;
            }
        }
    }else{
...
/* FUJITSU:2011-06-16 add mutex_lock/unlock for irq start */
	mutex_lock(&chip->pm_lock);
/* FUJITSU:2011-06-16 add mutex_lock/unlock for irq end */
	for (i = 0; i < handled; i++) {
