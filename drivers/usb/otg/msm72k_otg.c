...
unsigned char no_change_int_flag = 0;
...
extern int g_cradle_stat;
static int g_suspend_lock_stat = 0;
static int g_resume_lock_stat = 0;
/* FUJITSU:2011-07-07 USB end */
/* FUJITSU:2011-06-09 USB start */
...
static int is_b_sess_vld(void)
{
	...
	if(system_rev < 0x0c)
		...
		return is_b_sess_vld_a();
#endif /* CONFIG_MACH_F11EIF */
}
/* FUJITSU:2011-06-09 USB end */
...
/* FUJITSU:2011-07-18 USB start */
	if (!g_suspend_lock_stat){
		g_suspend_lock_stat=1;
/* FUJITSU:2011-07-18 USB end */
...
/* FUJITSU:2011-07-18 USB start */
	if (!g_resume_lock_stat){
		g_resume_lock_stat = 1;
	if (!atomic_read(&dev->in_lpm)){
		g_resume_lock_stat = 0;
		return 0;
	}
/* FUJITSU:2011-07-18 USB end */
...
/* FUJITSU:2011-08-05 USB start */
			disable_irq_nosync(dev->irq);
			wake_lock(&dev->wlock);
			queue_work(dev->wq, &dev->otg_resume_work);
			goto out;
/* FUJITSU:2011-08-05 USB end */
		}
#ifdef CONFIG_MACH_F11EIF
	}
#endif /* CONFIG_MACH_F11EIF */
...
/* FUJITSU:2011-07-01 USB start */
#ifdef CONFIG_MACH_F11EIF
	if (system_rev < 0x0c){
#endif /* CONFIG_MACH_F11EIF */
		if ( ReceivedCableDetectEvent == 1 ){
			pr_info("%s: event skip[%d]\n", __func__,ReceivedCableDetectEvent );
			return;
		}
#ifdef CONFIG_MACH_F11EIF
	}
#endif /* CONFIG_MACH_F11EIF */
/* FUJITSU:2011-07-01 USB end */
/* FUJITSU:2011-07-11 USB start */
	hsusb_notifi_usb_ctrl_power(1); /* USB POWER ON */
/* FUJITSU:2011-07-11 USB end */
...
/* FUJITSU:2011-06-15 USB start */
#ifdef CONFIG_MACH_F11EIF
	if (system_rev < 0x0c){
#endif /* CONFIG_MACH_F11EIF */
		if ( ReceivedCableDetectEvent == 0){
			pr_info("%s: otg_enable_irq for otg"
				"not received cable_detect_event!!!!\n", __func__);
			hsusb_notifi_usb_ctrl_power(0); /* USB POWER OFF */
			ReceivedCableDetectEvent = 0;
			return;
...
/* FUJITSU:2011-07-09 USB start */
			...
			if (no_change_int_flag != 0){
				printk(KERN_DEBUG "USB_DEBUG:%s  no_change_int_flag[%d]\n",__func__,no_change_int_flag); 
				no_change_int_flag = 0;
				msm_otg_set_suspend(&dev->otg, 1);
			}else{
				msm_hsusb_set_vbus_state(0);
			}
			printk(KERN_DEBUG "USB_DEBUG:%s suspend\n",__func__); 
/* FUJITSU:2011-07-09 USB end */
		}
/* FUJITSU:2011-07-18 USB start */
		}else{
