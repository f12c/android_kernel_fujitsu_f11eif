...
static void dbg_print(u8 addr, const char *name, int status, const char *extra)
{
	...
	if (dbg_data.tty != 0)
		pr_notice("%04X\tサ %02X %-7.7s %4i ォ\t%s\n",
			  stamp, addr, name, status, extra);
}
			   ...
			   char *buf)
{
	...
	n += scnprintf(buf + n, PAGE_SIZE - n, "*test = %d\n",
		       isr_statistics.test);
	n += scnprintf(buf + n, PAGE_SIZE - n, "サ ui  = %d\n",
		       isr_statistics.ui);
	n += scnprintf(buf + n, PAGE_SIZE - n, "サ uei = %d\n",
		       isr_statistics.uei);
	n += scnprintf(buf + n, PAGE_SIZE - n, "サ pci = %d\n",
		       isr_statistics.pci);
	n += scnprintf(buf + n, PAGE_SIZE - n, "サ uri = %d\n",
		       isr_statistics.uri);
	n += scnprintf(buf + n, PAGE_SIZE - n, "サ sli = %d\n",
		       isr_statistics.sli);
